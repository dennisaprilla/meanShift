%-------------------------------------
function z = compute_meanshift_vector(imPatch, ROI_Center, weights)
...
for x=1:size(imPatch,1)
    for y=1:size(imPatch,2)
        ...
        sum_x = sum_x + (x*g)*pixel_weight;
        sum_y = sum_y + (y*g)*pixel_weight;
    end
end

coordinate = ROI_Center - size(imPatch)/2;
